import React from 'react'
import { Fade } from 'react-reveal'
import { Brew } from '../../data/MenuData'

const BoxSection3 = () => {
  return (
    <Fade>
        <section className='mt-5 grid grid-cols-1 md:grid-cols-2 gap-3 bg-[#f06464]'>
        <img src={Brew.image} className="w-full md:order-last md:my-5" alt="banner"/>
        <div className='max-w-[500px] text-center justify-self-center px-[9px] md:px-[20px]'>
          <h2 className='md:text-5xl text-[#fafafa] font-extrabold text-lg md:max-w-[28rem] md:ml-[10%] md:font-semibold mb-[20px] tracking-[6px] uppercase'>{Brew.heading}</h2>
          <p className='text-[24px] text-[#fafafa] font-semibold md:max-w-[28rem] md:ml-[10% mb-[20px]'>{Brew.text}</p>
          <div>
            <button className='border-[#fafafa] border-2 text-[#fafafa] mb-5 px-4 py-2 rounded-full'>{Brew.button}</button>
          </div>
        </div>
      </section>
    </Fade>
  )
}

export default BoxSection3